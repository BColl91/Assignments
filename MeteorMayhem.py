...
class Asteroid(Wrapper):
    ...
    def die(self):
        ...
        if Asteroid.total == 0 and Comet.total == 0:
            self.game.advance()
...
class Comet(Wrapper):
    image = games.load_image("Comet.png")
    SPEED = 1
    SPAWN = 1
    POINTS = 50
    total = 0
    ...
    def __init__(self, game, x, y):
        ...
        dx = random.choice([1, -1]) * Comet.SPEED * random.random()
        dy = random.choice([1, -1]) * Comet.SPEED * random.random()

        super(Comet, self).__init__(image=Comet.image, x=x, y=y, dx=dx, dy=dy)
        self.game = game
        self.angle = math.degrees(angle) + 90

    def update(self):
        super(Comet, self).update()
        self.x += self.dx
        self.y += self.dy

    def die(self):
        Comet.total -= 1
        self.game.score.value += Comet.POINTS
        self.game.score.right = games.screen.width - 10
    ...
    def advance(self):
        ...
        for i in range(self.level):
            while True:
                ...
                if abs(x - self.ship.x) > BUFFER and abs(y - self.ship.y) > BUFFER:
                    break
